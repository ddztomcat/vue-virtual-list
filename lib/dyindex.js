export default {
  ...
  created() {
    ...
    // 防止抖动
    this.judgeReachBottom = this.getDebounce(this.judgeReachBottom, 60)
  },
  methods: {
    ...
    judgeReachBottom(lsp, sp) {
      if (
        this.__index + this.keep >= this.total - 1 &&
        sp > lsp
      ) {
    ...
    scroll(e) {
      ...
      this.__index = index
      console.log(index)
      this.judgeReachBottom(this.__lastScrollTop, sp)
      if (
